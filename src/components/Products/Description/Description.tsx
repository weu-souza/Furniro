...
const Description = ({ product }: descriptionType) => {
  return (
    <div className="max-w-screen-xl flex flex-col gap-9">
      <div className="flex mx-auto justify-between max-w-[800px]  w-full">
      ...
      <div className="flex mx-auto">
        {product.images.gallery.slice(0, 2).map((imgs, index) => (
          <img className="w-[605px] h-[348px] rounded-xl object-cover" src={imgs} key={index} alt={product.title} />
        ))}
      </div>
    </div>
  );
