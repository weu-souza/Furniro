  ...
  return (
    <header className=' flex flex-wrap md:justify-between py-7 max-w-screen-xl mx-auto items-center' onClick={closeCart}>
     ...
     <div className='flex gap-5 items-center self-end'>
      ...
      <div >
      <button className="relative py-3 px-5 rounded-full "  onClick={(e)=> OpenCart(e)}><CartIcon/> {storage > 0 && <p className='absolute font-Poppins font-bold text-sm text-cor-D9D9D9 bg-cor-B88E2F  px-2 rounded-full top-0 right-0'>{storage}</p>} </button>
